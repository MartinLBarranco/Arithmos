...
class InferenceRule:
    ...
    def variables(self) -> Set[str]:
        ...
        """
        varis = set()
        for formu in self.assumptions:
            varis.update(formu.variables())
        varis.update(self.conclusion.variables())
        return varis
        # Task 4.1
    ...
    def specialize(self, specialization_map: SpecializationMap) -> \
        ...
        if specialization_map == {}:
            return self
        # Sustituimos en las premisas
        nuevas_premisas = []
        for premisa in self.assumptions:
            #premisa = premisa.substitute_variables(specialization_map)
            nuevas_premisas.append(premisa.substitute_variables(specialization_map))
        # Sustituimos en la conclusión
        conclusion = self.conclusion.substitute_variables(specialization_map)
        return InferenceRule(nuevas_premisas, conclusion)
        # Task 4.4
