...
class Formula:
    ...
    def variables(self) -> Set[str]:
        ...
        if is_variable(self.root):
            ...
            assert is_binary(self.root)
            assert self.first is not None and self.second is not None
            variables.update(self.first.variables(), self.second.variables())
        return variables
    ...
    def operators(self) -> Set[str]:
        ...
        if is_variable(self.root):
            ...
            operadores.update({self.root})
        elif is_unary(self.root):
            assert self.first is not None
            operadores.update(self.root)
            operadores.update(self.first.operators())
        else:
            assert is_binary(self.root)
            assert self.first is not None and self.second is not None
            operadores.update({self.root})
            operadores.update(self.first.operators(), self.second.operators())
        return operadores
